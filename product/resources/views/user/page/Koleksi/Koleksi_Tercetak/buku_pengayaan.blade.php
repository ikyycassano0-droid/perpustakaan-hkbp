{{-- resources/views/user/page/koleksi_tercetak/buku_pengayaan.blade.php --}}
@extends('user.component.master')

@section('title', 'Koleksi Buku Pengayaan - Perpustakaan AKPER HKBP')

@push('styles')
<style>
    /* ============================================
       GAYA BUKU PENGAYAAN (TEMA HIJAU KUNING)
    ============================================ */
    :root {
    ...
    .book-info-main .meta-text {
        font-size: 0.9rem;
        color: var(--text-muted);
        margin-bottom: 4px;
    }

    .book-info-main .details-grid {
        display: grid;
...
<div class="main-container">
    ...
    <main class="content-section">
        <div class="page-header">
            ...
            <h2>Koleksi Buku Pengayaan</h2>
        </div>

        <!-- Form Search & Filter (server-side) -->
        <form method="GET" action="{{ route('user.koleksi.buku_pengayaan') }}" class="filter-row">
        ...
        <div class="filter-container">
            <a href="{{ route('user.koleksi.buku_pengayaan', array_merge(request()->except('category'), ['category' => ''])) }}"
               class="chip {{ !request('category') ? 'active' : '' }}">Semua Kategori</a>
            @foreach($categories as $cat)
                <a href="{{ route('user.koleksi.buku_pengayaan', array_merge(request()->except('category'), ['category' => $cat->id])) }}"
                   class="chip {{ request('category') == $cat->id ? 'active' : '' }}">{{ $cat->name }}</a>
            @endforeach
        </div>

        <!-- Daftar Buku Pengayaan -->
        <div class="book-list">
            @forelse($collections as $item)
                @php
                    ...
                    $categoryName = $item->categories->isNotEmpty() ? $item->categories->first()->name : 'Buku Pengayaan';
                    $keywords = $item->keywords;
                    if (is_array($keywords)) {
                        $keywords = implode(', ', $keywords);
                    }

                    // ========== LOGIKA PEMINJAMAN YANG DITINGKATKAN ==========
                    $buttonText = 'Pinjam';
                    $buttonClass = 'btn-ui';
                ...
                <div class="book-item">
                    <div class="book-cover-box">
                        <img src="{{ $item->cover_image ? asset('storage/'.$item->cover_image) : 'https://via.placeholder.com/140x200?text=Buku' }}" alt="Cover">
                    </div>
                    <div class="book-info-main">
                        <span class="category-label">{{ $categoryName }}</span>
                        <h2 onclick="location.href='{{ route('user.koleksi.detail', $item->id) }}'">{{ $item->title }}</h2>
                        <p class="meta-text">Oleh: <strong>{{ $authorStr ?: 'Tim Penulis' }}</strong></p>
                        <p class="meta-text">Penerbit: <strong>{{ $item->publisher ?? 'AKPER HKBP Press' }} -- {{ $item->city ?? 'Tarutung' }}, {{ $item->publication_year ?? date('Y') }}</strong></p>

                        <div class="details-grid">
                            <span><strong>ISBN:</strong> {{ $item->isbn ?? '-' }}</span>
                            <span><strong>Edisi:</strong> {{ $item->edition ?? 'Cetakan Pertama' }}</span>
                            <span><strong>Subjek:</strong> {{ $item->subject ?? ($keywords ?? '-') }}</span>
                            <span><strong>Klasifikasi:</strong> {{ $item->classification ?? '-' }}</span>
                        </div>
                    </div>
                    <div class="book-side-actions">
                        <div class="stock-indicator">
                            <span>Ketersediaan</span>
                            <strong>{{ $item->available_stock ?? 0 }}</strong>
                        </div>
                        @if($item->marc_file)
                            <button class="btn-ui" onclick="window.open('{{ asset('storage/'.$item->marc_file) }}', '_blank')">Unduh MARC</button>
                        @endif
                        <button class="btn-ui" onclick="showCitation('{{ addslashes($item->title) }}', '{{ addslashes($authorStr) }}', '{{ addslashes($item->publisher) }}', '{{ $item->publication_year }}')">Sitasi</button>

                        <div class="btn-wrapper">
                            @if($buttonOnclick && !$buttonDisabled)
                                <button onclick="{{ $buttonOnclick }}" class="{{ $buttonClass }}" {{ $buttonDisabled ? 'disabled' : '' }}>
                                    {{ $buttonText }}
                                </button>
                            @else
            ...
            @empty
                <div class="text-center py-10 text-gray-500">Belum ada buku pengayaan yang tersedia.</div>
            @endforelse
        </div>
...
<!-- Modal Peminjaman (classic style with enhanced logic) -->
<div id="pinjamModal" class="modal-overlay" style="position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(12px); z-index: 1000; display: flex; align-items: center; justify-content: center; visibility: hidden; opacity: 0; transition: all 0.3s ease;">
    <div class="modal-container" style="background: rgba(15,23,42,0.95); backdrop-filter: blur(16px); border: 1px solid rgba(99,102,241,0.5); border-radius: 1.5rem; width: 100%; max-width: 28rem; margin: 1rem; transform: scale(0.9); transition: transform 0.3s ease; padding: 1.5rem;">
        <h2 class="text-xl font-bold text-indigo-300 mb-4">Form Peminjaman Buku</h2>
...
<script>
    ...
    // Validasi tanggal kembali ketika tanggal pinjam berubah
