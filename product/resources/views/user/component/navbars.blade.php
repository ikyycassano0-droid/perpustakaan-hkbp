    ...
    <div style="display: flex; align-items: center; gap: 15px;">
        @if(session()->has('user'))
            @php
                ...
                // Ambil foto dari database jika ada
                $userPhoto = \App\Models\User::find($userId)->photo ?? null;
                $userName = session('user')['name'] ?? 'Profil';
            ...
            {{-- Tombol Profil dengan Foto + Nama --}}
            <div class="profile-dropdown-wrapper">
                <button class="profile-btn" aria-label="Menu Profil">
                    <span class="profile-avatar">
                        @if($userPhoto)
                            <img src="{{ asset('storage/' . $userPhoto) }}" alt="Foto Profil" class="avatar-img">
                        @else
...
<style>
    ...
    @media (max-width: 1100px) {
        nav ul { gap: 2px; }
        nav ul li a { font-size: 0.75rem; padding: 10px 8px; }
    }
