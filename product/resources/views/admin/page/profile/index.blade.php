...
<div class="max-w-7xl mx-auto">
    ...
    {{-- ALERT SUCCESS & ERROR --}}
    @if(session('success'))@extends('admin.component.main')
...
const existingDeskripsi = {{ \App\Models\Profile::where('type', 'kerjasama')->where('sub_type', 'deskripsi')->where('active', true)->exists() ? 'true' : 'false' }};
...
function ubahForm() {
    ...
    if (type === 'visi_misi') {
        ...
        const jabatanLabel = fieldJabatan.querySelector('label');
        if (jabatanLabel) jabatanLabel.innerText = 'Jabatan';
        fieldIcon.classList.add('hidden');
        descTA.placeholder = 'Kontak atau deskripsi singkat';
    }
    else if (type === 'kerjasama') {
        // DESKRIPSI - hanya tampil jika belum ada (HANYA BOLEH 1)
        if (!existingDeskripsi) {
            subType.innerHTML += `<option value="deskripsi">Deskripsi (Jaringan Mitra Strategis)</option>`;
        }
        // MITRA - SELALU TAMPIL (BOLEH BANYAK)
        subType.innerHTML += `<option value="mitra">Mitra</option>`;
        subType.innerHTML += `<option value="bentuk">Bentuk Kerjasama</option>`;
        subType.innerHTML += `<option value="kolaborasi">Kolaborasi</option>`;
...
function ubahSubType() {
    ...
    if (type === 'kerjasama' && subVal === 'kolaborasi') {
    ...
    else if (type === 'kerjasama' && subVal === 'deskripsi') {
        // Deskripsi: hanya judul dan deskripsi (tanpa gambar, icon, order)
        fieldTitle.classList.remove('hidden');
        fieldJabatan.classList.add('hidden');
        fieldDesc.classList.remove('hidden');
        fieldIcon.classList.add('hidden');
        ...
        fieldOrder.classList.add('hidden');
        labelTitle.innerText = 'Judul Section';
    }
    else {
        // Mitra dan Bentuk: normal (semua field kecuali jabatan)
        fieldTitle.classList.remove('hidden');
        fieldDesc.classList.remove('hidden');
        fieldIcon.classList.remove('hidden');
        fieldOrder.classList.remove('hidden');
...
function openEditModal(id) {
    ...
    const isDeskripsi = (profile.type === 'kerjasama' && profile.sub_type === 'deskripsi');
    ...
    if (isKolaborasi) {
        ...
        document.getElementById('edit_label_title').innerText = 'Judul Section';
