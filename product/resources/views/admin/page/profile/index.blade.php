...
@section('content')
                ...
                <form action="{{ route('admin.profile.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div id="fieldJabatan" class="hidden">
                            <label class="block text-sm font-medium text-slate-700 mb-2" id="labelJabatan">Jabatan /
                                Peran</label>
                            <input type="text" name="jabatan" id="jabatan_input"
                                placeholder="Contoh: Direktur / Institusi Mitra"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                        </div>
                    ...
                    <thead>
                        <tr class="bg-slate-50/80 border-b border-slate-100">
                            ...
                            @if (in_array(request()->type, ['visi_misi', 'tugas_fungsi', 'kerjasama']))
                                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Isi
                                    (Deskripsi)</th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Jabatan
                                </th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Gambar
                                </th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Urutan
                                </th>
                            @else
                                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Judul</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Jabatan
                                </th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Gambar
                                </th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Urutan
                        ...
                        @foreach ($profiles as $item)
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50/30 transition">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($item->active)
                                        <span
                                            class="inline-flex px-2 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">
                                            <i class="fas fa-check-circle text-[10px] mr-1"></i> Aktif
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex px-2 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-500">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <button type="button"
                                            class="btn-edit w-8 h-8 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 transition flex items-center justify-center"
                                            data-id="{{ $item->id }}">
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="fixed inset-0 bg-slate-900 bg-opacity-50 transition-opacity" onclick="closeEditModal()">
            </div>
                ...
                <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header border-b border-slate-100 px-6 py-4">
                        ...
                        <button type="button" onclick="closeEditModal()"
                            class="absolute right-6 top-4 text-slate-400 hover:text-slate-600 transition">
                    ...
                    <div class="modal-body p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="edit_type_display"
                        ...
                        <div id="edit_order_container">
                            <label class="block text-sm font-medium text-slate-700 mb-2">Urutan</label>
                            <input type="number" id="edit_order" name="order" min="1"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Status</label>
    ...
    <script>
        ...
        function ubahForm() {
            ...
            if (type === 'visi_misi') {
                ...
                subType.innerHTML +=
                    `<option value="tugas">Tugas</option><option value="fungsi">Fungsi</option><option value="tujuan">Tujuan</option>`;
