...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <button onclick="openCreateModal()"
            class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm transition flex items-center gap-2 w-fit">
                    ...
                    <tr class="hover:bg-slate-50 transition group" data-id="{{ $schedule->id }}">
                        ...
                        <td class="px-6 py-4">
                            <span class="editable-status" data-field="status" data-id="{{ $schedule->id }}">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium status-badge-{{ $schedule->id }}"
                                      style="background-color: {{ $schedule->status_color }}20; color: {{ $schedule->status_color }}">
                        ...
                        <td class="px-6 py-4">
                            <select class="editable-select text-sm px-2 py-1 rounded-lg border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition"
                                    data-field="status_color" data-id="{{ $schedule->id }}"
                                    style="background-color: {{ $schedule->status_color }}20;">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="openEditModal({{ json_encode($schedule) }})"
                                    class="text-indigo-600 hover:text-indigo-800 transition" title="Edit Lengkap">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick="confirmDelete({{ $schedule->id }}, '{{ $schedule->day_name }}')"
    ...
    <div class="bg-white rounded-2xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('admin.waktu_layanan.store') }}" method="POST" class="p-5">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="day_short" required maxlength="5" placeholder="Sn"
                    ...
                    <div>
                        ...
                        <input type="text" name="day_name" required maxlength="20" placeholder="Senin"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Jam Layanan <span class="text-red-500">*</span></label>
                    <input type="text" name="service_hours" required placeholder="08:00 — 16:30"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="status" required placeholder="Layanan Penuh"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Warna Status <span class="text-red-500">*</span></label>
                        <select name="status_color" required
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none">
                ...
                <div>
                    ...
                    <textarea name="note" rows="2" placeholder="Contoh: Sirkulasi Aktif, Layanan terbatas, dll"
                ...
                <div>
                    ...
                    <input type="number" name="order" required min="0" value="0"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
            ...
            <div class="flex gap-3 mt-6 pt-3">
                ...
                <button type="submit"
                    class="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition shadow-sm">Simpan</button>
            </div>
        </form>
    </div>
</div>
        ...
        <form id="editForm" method="POST" class="p-5">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="day_short" id="edit_day_short" required maxlength="5"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Nama Hari</label>
                        <input type="text" name="day_name" id="edit_day_name" required maxlength="20"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Jam Layanan</label>
                    <input type="text" name="service_hours" id="edit_service_hours" required
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="status_color" id="edit_status_color" required
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none">
                            <option value="#10b981">🟢 Hijau (Emerald)</option>
                            <option value="#f59e0b">🟡 Kuning (Amber)</option>
                            <option value="#f97316">🟠 Orange</option>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="edit_order" required min="0"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                </div>
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
            ...
            <div class="flex gap-3 mt-6 pt-3">
                <button type="button" onclick="closeEditModal()"
                    class="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 transition">Batal</button>
                <button type="submit"
                    class="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition shadow-sm">Update</button>
            </div>
        </form>
    </div>
</div>
