            ...
            <form action="{{ route('admin.profile.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" onchange="ubahForm()" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    ...
                    <div>
                        ...
                        <select name="sub_type" id="sub_type" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    ...
                    <div id="fieldTitle">
                        ...
                        <input type="text" name="title" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    </div>

                    {{-- JABATAN --}}
                    <div id="fieldJabatan" class="hidden">
                        <label class="block text-sm font-medium text-slate-700 mb-2">Jabatan</label>
                        <input type="text" name="jabatan" placeholder="Contoh: Direktur" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    </div>

                    {{-- DESKRIPSI --}}
                    <div id="fieldDesc" class="md:col-span-2 lg:col-span-4">
                        <label class="block text-sm font-medium text-slate-700 mb-2">Deskripsi</label>
                        <textarea name="description" id="descriptionTextarea" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30"></textarea>
                    </div>

                    {{-- ICON --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Icon</label>
                        <input type="text" name="icon" placeholder="fas fa-building" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    </div>

                    {{-- GAMBAR --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Gambar</label>
                        <input type="file" name="image" accept="image/*" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    </div>

                    {{-- ORDER --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Urutan</label>
                        <input type="number" name="order" value="1" min="1" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/30">
                    </div>
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="w-full px-5 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition">
                            Tambah Data
                        </button>
                    </div>
                    ...
                    <tr class="border-b border-slate-50 hover:bg-slate-50/30 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button type="button" data-bs-toggle="modal" data-bs-target="#editModal{{ $item->id }}" class="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 transition flex items-center justify-center">
            ...
            <div class="modal-content rounded-2xl border-0 shadow-2xl">
                <form action="{{ route('admin.profile.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body p-6 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Judul</label>
                            <input type="text" name="title" value="{{ $item->title }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Jabatan</label>
                            <input type="text" name="jabatan" value="{{ $item->jabatan }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Deskripsi</label>
                            <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200">{{ $item->description }}</textarea>
                        </div>
                    </div>

                    <div class="modal-footer border-t border-slate-100 px-6 py-4 flex justify-end gap-3">
                        <button type="button" data-bs-dismiss="modal" class="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-50 transition">Batal</button>
                        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
...
// Fungsi utama untuk mengubah form berdasarkan type yang dipilih
function ubahForm() {
    ...
    let descTextarea = document.getElementById('descriptionTextarea');

    // Reset sub type options
    subType.innerHTML = '<option value="">-- Pilih Sub Type --</option>';

    // Sembunyikan jabatan dulu
    ...
    if(type === 'visi_misi'){
        ...
        descTextarea.placeholder = 'Masukkan deskripsi Visi/Misi/About';
    }
        ...
        `;
        title.querySelector('label').innerText = 'Judul';
        descTextarea.placeholder = 'Masukkan deskripsi Tugas/Fungsi/Tujuan';
    }
        ...
        descTextarea.placeholder = 'Masukkan deskripsi kerjasama';
    }
    else {
        // Default
        title.querySelector('label').innerText = 'Judul';
...
document.addEventListener('DOMContentLoaded', function() {
    // Set default value jika type sudah terisi
